...
CHeightImage::CHeightImage(LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale)
{
	m_nWidth = nWidth;
	m_nLength = nLength;
...
float CHeightImage::GetHeight(float fx, float fz)
{
...
CHeightMesh::CHeightMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
...
{
	CHeightImage* pHeightMapImage = (CHeightImage*)pContext;
...
float CHeightMesh::OnGetHeight(int x, int z, void* pContext)
{
	CHeightImage* pHeightMapImage = (CHeightImage*)pContext;
...
XMFLOAT4 CHeightMesh::OnGetColor(int x, int z, void* pContext)
{
...
CHeightMesh::~CHeightMesh()
{
}

XMFLOAT3 CHeightImage::GetHeightMapNormal(int x, int z)
{
	if ((x < 0.0f) || (z < 0.0f) || (x >= m_nWidth) || (z >= m_nLength))

		return(XMFLOAT3(0.0f, 1.0f, 0.0f));
	int HeightIndex = x + (z * m_nWidth);
	int HeightAdd_X = (x < (m_nWidth - 1)) ? 1 : -1;
	int HeightAdd_Z = (z < (m_nLength - 1)) ? m_nWidth : -m_nWidth;
	float y1 = (float)m_pHeightMapPixels[HeightIndex] * m_xmf3Scale.y;
	float y2 = (float)m_pHeightMapPixels[HeightIndex + HeightAdd_X] * m_xmf3Scale.y;
	float y3 = (float)m_pHeightMapPixels[HeightIndex + HeightAdd_Z] * m_xmf3Scale.y;
